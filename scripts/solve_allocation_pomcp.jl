...
no_of_scenarios = 1
CreateGroundTruthSmooth(no_of_scenarios, amount_of_scatters=50, grid_size=(50,50), limit_range=0:100)
global all_scenarios_ground_truth = ReadGroundTruth(no_of_scenarios)

for itr in 1:no_of_scenarios
    println("### Scenario $itr ###")
    scenario_ground_truth = all_scenarios_ground_truth[:,:,itr]
    ...
    writedlm("./ScenarioOutputs/rewards_scenario$itr.txt", rewards)
    rewards = sum(rewards)
